...
extract "$MY_DIR/broken-proprietary-blobs.txt" "$SRC"
...
if [ "$SRC" != "adb" ]; then
  for file in $(egrep -v '(^#|^$)' "$MY_DIR"/broken-proprietary-blobs-vendorimg.txt); do
...
extract "$MY_DIR/broken-proprietary-blobs-vendorimg.txt" "$SRC"
